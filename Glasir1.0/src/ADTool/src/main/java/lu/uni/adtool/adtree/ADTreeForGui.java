 ...
 * {@link #ADTreeForGui(Object)}. Then you can incrementally
...
public class ADTreeForGui extends AbstractTreeForTreeLayout<ADTreeNode> implements Serializable
{
   ...
   * @param newChildren map from nodes to children
   * @param newParents map from nodes to parents
   * @param newViewRoot root after folding from above.
   *
   */
   ...
   *
   * @param node node
   * @param ignoreFold if true we return parent even if the node is folded from
   * above
   ...
   *
   * @param node node for which operation is toggled.
   */
   ...
   *
   * @param index index at which we add new node.
   * @param parent parent of the node.
   * @param node node to be added.
   */
  private void addToLevelList(final int index, final ADTreeNode parent,
      final ADTreeNode node)
  {
    if (parent.isFolded()){
      return;
    }
    node.setLevel(parent.getLevel()+1);
    final List<ADTreeNode> nodeLevelList
      = getLevelList(node.getLevel());
   ...
   * @param node node to be added.
   * @param counter whether we add a counter node.
   */
  private void addToLevelList(final ADTreeNode parent, final ADTreeNode node,
      final boolean counter)
  {
    ...
    if (childList.size()==0){
      ...
      if (parentLevelList.size()>1){
        final ListIterator<ADTreeNode> iter
          = parentLevelList.listIterator(parentLevelList.indexOf(parent));
   ...
   * @param node
   *          [!hasNode(node)]
   * @param notify whether to notify about tree change.
   */
  ...
  /**
   * Removes a node from a tree. Note that root cannot be removed.
   * TODO: update values of levels.
   * @param node
   */
   ...
   *
   * @param node a root of a subtree to be removed.
   */
   ...
   * @param id initial id for a term for current node.
   * @return
   */
  public ADTNode getTerms(ADTreeNode node, int id)
  {
    ...
    if (list.size() == 0){
      if (id == 0) {
        ...
        //if ( getParent(node,true).getType() == ADTreeNode.Type.PROPONENT) {
   ...
   *
   * @param extendProvider The extendProvider.
   */
   ...
   *
   * @param node node to be folded from above.
   */
   ...
   *
   * @param localExtendProvider The localExtendProvider.
   */
