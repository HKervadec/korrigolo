\section{Introduction}
    ...
    Le projet décrit dans ce rapport consiste à développer un logiciel d'analyse informatique des ADTrees\footnote{Abréviation d'\og Attack-Defense Trees \fg{}, ou \og Arbres d'Attaque et de Défense\fg{} en français.}. Ce logiciel, baptisé Glasir et dont le logo est visible sur la {\sc Figure}~\ref{fig:glasir}, intègre ADTool~\cite{ADTool}, un outil d'édition d'ADTrees déjà existant. ADTool est utilisé dans Glasir pour l'affichage, la création et l'édition des ADTrees, et doit subir quelques modifications destinées à améliorer son ergonomie. Glasir, pour sa part, fournira les fonctionnalités d'analyse suivantes :
