...
\BK{vous pouvez supprimer la partie entre parenthèses}
...
\BK{Très bien. La partie sur les tests se lit maintenant bien mieux qu'avant.}
...
\BK{Il n'est pas clair pour moi ce que vous voulez dire par 'affichage séparé'}
...
La seule critique faite à l'égard du couper/copier/coller était que le menu déroulant comportait toujours la possibilité de coller \BK{(sans aucun effet)} un n\oe{}ud même si aucun n'avait été préalablement copié ni coupé (sans aucun effet) \BK{déplacez (sans aucun effet)}. Bien que sans conséquences notables, cette petite incohérence fut vite corrigée. 

Il a également été demandé après les tests de la version 0.2 d'ajouter en haut de la fenêtre d'ADTool le nom (et même le path complet) de l'ADTree affiché, ce qui a été \BK{'fait' à la place d'ajouté depuis} ajouté depuis. C'est en effet très pratique lorsque plusieurs instances d'ADTool sont ouvertes simultanément, comme c'est souvent le cas lors de l'utilisation de Glasir. 


\BK{Cette section s'est également beaucoup améliorée. Il est très clair maintenant 
que les remarques ont été implémentées.}
