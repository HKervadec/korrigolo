\section{ADTool : étude de l'existant}
	...
	L'utilisateur n'a donc pas de calcul à faire lui-même pour obtenir la valuation de la racine de son arbre, c'est-à-dire la valuation de son objectif final.	Cependant, ADTool ne précise pas d'où provient la valuation d'un nœud.

	Cette étude mets donc en évidence deux % CHECK
	limitations majeurs d'ADTools :  
	...
	À l'issue de cette étude d'ADTool, certaines limitations ont été mises en avant car elles rendent l'analyse des arbres édités laborieuse. La prochaine partie détaillera l'architecture du logiciel Glasir, créer dans le but de dépasser* les limitations évoquées.