...
L'utilisateur est donc cantonné à ces valuations, et ne peut pas en créer d'autres sans passer par le code d'ADTool. Celui-ci étant libre de droits, c'est donc possible mais nous souhaitons donner la possibilité de créer des valuations à partir de celles déjà disponibles.

La fonction de sythèse prendrait donc en entrée :
		\begin{itemize}
			\item un arbre provenant du projet ;
			\item les paramètres intervenant dans la synthèse ;
			\item le nom de la nouvelle valuation ($min$ ou $max$).
		\end{itemize}
		%\begin{itemize}
